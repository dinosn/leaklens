...
func init() {
	...
	githubScanCmd.Flags().StringVar(&githubOutputPath, "output", "leaklens.db", "Output database path")
	...
	githubCmd.Flags().StringVar(&githubOutputPath, "output", "leaklens.db", "Output database path")
...
func runGitHubScan(cmd *cobra.Command, args []string) error {
	...
	if len(args) > 0 {
		...
		if len(parts) != 2 {
			return fmt.Errorf("invalid repository format, expected owner/repo (e.g., dinosn/leaklens)")
		}
