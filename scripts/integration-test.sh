#!/bin/bash
# Integration tests for LeakLens secrets scanner
# Runs scans against test fixtures and validates output

set -Eeuo pipefail

# Configuration
LEAKLENS="${LEAKLENS:-./leaklens}"
TESTDATA_DIR="${TESTDATA_DIR:-testdata/secrets}"
RESULTS_FILE="/tmp/leaklens-integration-results.json"
...
if [ ! -x "$LEAKLENS" ]; then
    fail "LeakLens binary not found at $LEAKLENS. Run 'make build' first."
fi
...
if ! $LEAKLENS scan "$TESTDATA_DIR" --format=human > /dev/null 2>&1; then
...
INCLUDE_FILE="/tmp/leaklens-include-results.json"
INCLUDE_DB="/tmp/leaklens-include.db"
rm -f "$INCLUDE_DB"
$LEAKLENS scan "$TESTDATA_DIR" --rules-include "np.aws.*" --output="$INCLUDE_DB" --format=json > "$INCLUDE_FILE" 2>/dev/null
...
EXCLUDE_FILE="/tmp/leaklens-exclude-results.json"
EXCLUDE_DB="/tmp/leaklens-exclude.db"
rm -f "$EXCLUDE_DB"
$LEAKLENS scan "$TESTDATA_DIR" --rules-exclude "np.aws.*" --output="$EXCLUDE_DB" --format=json > "$EXCLUDE_FILE" 2>/dev/null
...
RULES_LIST_FILE="/tmp/leaklens-rules-list.json"
$LEAKLENS rules list --include "np.aws.*" --format=json > "$RULES_LIST_FILE" 2>/dev/null
...
COMBINED_FILE="/tmp/leaklens-combined-results.json"
COMBINED_DB="/tmp/leaklens-combined.db"
rm -f "$COMBINED_DB"
$LEAKLENS scan "$TESTDATA_DIR" --rules-include "np\\..*" --rules-exclude ".*token.*" --output="$COMBINED_DB" --format=json > "$COMBINED_FILE" 2>/dev/null
...
CONTEXT_FILE="/tmp/leaklens-context-results.json"
CONTEXT_DB="/tmp/leaklens-context.db"
rm -f "$CONTEXT_DB"
$LEAKLENS scan "$TESTDATA_DIR" --context-lines=3 --output="$CONTEXT_DB" --format=json > "$CONTEXT_FILE" 2>/dev/null
...
NO_CONTEXT_FILE="/tmp/leaklens-no-context-results.json"
NO_CONTEXT_DB="/tmp/leaklens-no-context.db"
rm -f "$NO_CONTEXT_DB"
$LEAKLENS scan "$TESTDATA_DIR" --context-lines=0 --output="$NO_CONTEXT_DB" --format=json > "$NO_CONTEXT_FILE" 2>/dev/null
...
REPORT_DB="/tmp/leaklens-report.db"
rm -f "$REPORT_DB"

# Run scan to create datastore
$LEAKLENS scan "$TESTDATA_DIR" --output="$REPORT_DB" --format=json > /dev/null 2>&1
...
REPORT_OUTPUT="/tmp/leaklens-report-human.txt"
$LEAKLENS report --datastore="$REPORT_DB" --format=human > "$REPORT_OUTPUT" 2>&1

# Verify report contains expected sections
if ! grep -q "=== LeakLens Report ===" "$REPORT_OUTPUT"; then
...
REPORT_JSON="/tmp/leaklens-report.json"
$LEAKLENS report --datastore="$REPORT_DB" --format=json > "$REPORT_JSON" 2>&1
...
INCREMENTAL_DB="/tmp/leaklens-incremental.db"
rm -f "$INCREMENTAL_DB"

# First scan
$LEAKLENS scan "$TESTDATA_DIR" --output="$INCREMENTAL_DB" --format=human > /tmp/leaklens-inc1.txt 2>&1

# Second scan with incremental - should skip all blobs
$LEAKLENS scan "$TESTDATA_DIR" --output="$INCREMENTAL_DB" --format=human --incremental > /tmp/leaklens-inc2.txt 2>&1

# Verify second scan mentions skipped blobs
if ! grep -q "skipped" /tmp/leaklens-inc2.txt; then
    fail "Expected incremental scan to report skipped blobs"
fi

pass "Incremental scanning works"
rm -f "$INCREMENTAL_DB" /tmp/leaklens-inc1.txt /tmp/leaklens-inc2.txt

# Test 14: SARIF format
echo "=== Test 14: SARIF Format ==="
SARIF_FILE="/tmp/leaklens-sarif-results.json"
SARIF_DB="/tmp/leaklens-sarif.db"
rm -f "$SARIF_DB"
$LEAKLENS scan "$TESTDATA_DIR" --output="$SARIF_DB" --format=sarif > "$SARIF_FILE" 2>/dev/null
...
# Verify tool name
if ! jq -e '.runs[0].tool.driver.name == "leaklens"' "$SARIF_FILE" > /dev/null 2>&1; then
    fail "SARIF tool name is not leaklens"
fi
