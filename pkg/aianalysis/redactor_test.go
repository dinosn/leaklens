...
func TestRedactorExpandedPreservesEndpointPathForManualCurl(t *testing.T) {
	redactor := NewRedactor(CloudRedactionExpanded, []string{"https://files-app.example.test/"})

	got := redactor.RedactContent(`
fetch("https://files-app.example.test/api/files/upload", {method: "POST"});
fetch("https://files-app.example.test/api/files/download?file_id=12345");
`)

	for _, forbidden := range []string{
		"files-app.example.test",
