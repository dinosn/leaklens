...
// TestNoseyParkerParity verifies that LeakLens finds all secrets that NoseyParker v0.24.0 finds.
// This test ensures we don't regress when porting from the original Rust implementation.
//
// NoseyParker v0.24.0 findings on html-secrets.html:
...
func TestNoseyParkerParity_HTMLTestFile(t *testing.T) {
	...
	// Read test file
	content, err := os.ReadFile("../../testdata/secrets/html-secrets.html")
	require.NoError(t, err)
	...
	// Verify LeakLens finds MORE than NoseyParker (additional rules)
	totalFindings := len(seenStructural)
	assert.GreaterOrEqual(t, totalFindings, 15,
		"LeakLens should find at least as many secrets as NoseyParker (15), found %d", totalFindings)
...
func TestNoseyParkerParity_MixedSecrets(t *testing.T) {
	...
	// NoseyParker finds 12 on this file, LeakLens should find at least that many
	totalFindings := len(seenStructural)
...
// BenchmarkLeakLensVsNoseyParkerSpeed provides benchmark data for comparison.
// Run with: go test -bench=BenchmarkLeakLensVsNoseyParkerSpeed -benchmem
//
...
//
// LeakLens non-CGO typical results:
// - Small files: Comparable to NoseyParker
// - Large files: 3-5x slower (trade-off for no CGO)
func BenchmarkLeakLensVsNoseyParkerSpeed(b *testing.B) {
	// Load rules once
	loader := rule.NewLoader()
	rules, err := loader.LoadBuiltinRules()
	if err != nil {
